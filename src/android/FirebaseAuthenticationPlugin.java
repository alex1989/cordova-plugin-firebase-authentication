...
public class FirebaseAuthenticationPlugin extends ReflectiveCordovaPlugin implements OnCompleteListener, AuthStateListener {
    ...
    public void onComplete(Task task) {
        if (this.signinCallback != null) {
            if (task.isSuccessful()) {
                ...
                this.signinCallback.success(getProfileData(
    ...
    public void onAuthStateChanged(FirebaseAuth auth) {
        if (this.authStateCallback != null) {
            ...
            if (user != null) {
                pluginResult = new PluginResult(PluginResult.Status.OK, getProfileData(user, null));
            } else {
    ...
    private static JSONObject getProfileData(FirebaseUser user, AdditionalUserInfo userInfo) {
        ...
        try {
            ...
            for(int i=0; i < providerData.size(); i++) {
                String id = providerData.get(i).getProviderId();
                if (i != 0) {
                  providerId += ", ";
                }
                providerId += id;
            ...
            result.put("isUserInfo", userInfo != null);
            result.put("isNewUser", userInfo != null ? userInfo.isNewUser() : null);
