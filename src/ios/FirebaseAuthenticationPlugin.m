...
static FIRAuthStateDidChangeListenerHandle handle = nil;
...
- (void)setAuthStateChanged:(CDVInvokedUrlCommand*)command {
    ...
    if (disable) {
        ...
        NSString* authStateChange = command.callbackId;
        if (handle != nil) {
            return;
        }
        handle = [[FIRAuth auth]addAuthStateDidChangeListener:^(FIRAuth *_Nonnull auth, FIRUser *_Nullable user) {
            CDVPluginResult *pluginResult;
            if (user) {
                pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:[self userToDictionary:user additionalUserInfo:nil]];
            } else {
                pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:@""];
            }
            [pluginResult setKeepCallbackAsBool:YES];
            [self.commandDelegate sendPluginResult:pluginResult callbackId:authStateChange];
        }];
    }
}
...
- (CDVPluginResult*) createAuthResult:(FIRAuthDataResult*)result withError:(NSError*)error {
    ...
    if (error) {
        ...
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:[self userToDictionary:result.user additionalUserInfo:result.additionalUserInfo]];
    }
    return pluginResult;
}

- (NSDictionary*)userToDictionary:(FIRUser *)user additionalUserInfo:(FIRAdditionalUserInfo *)additionalUserInfo {
    NSString * providers = [
        [user.providerData valueForKey:@"providerID"]
        componentsJoinedByString:@", "
    ];
    return @{
        @"uid": user.uid,
        @"providerId": providers,
        @"isNewUser": [additionalUserInfo isEqual:nil] ? @(FALSE) : @(additionalUserInfo.newUser),
